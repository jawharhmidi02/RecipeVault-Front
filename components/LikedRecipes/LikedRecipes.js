...
const LikedRecipes = ({ user, openRecipe }) => {
    ...
    if (user.id) {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/recipelikes/user/${user.id}?page=${CurrentPage}&limit=${limit}`,
          {
            method: "GET",
            headers: {
  ...
  return (
    ...
    >
      ...
      {!loadingRecipes && likedRecipes.length > 0 && (
